...
def get_args():
    ...
    parser.add_argument('--mct', dest="mct", action="store_true", help="Print mct Results or not")
...
def print_results(summary, mread=False):
    ...
    summary = summary[['idf1', 'idtp', 'idfp', 'idfn']]
    summary['idf1'] *= 100
    ...
    def compare_dataframes_mtmc(gts, ts):
        ...
        for k in sorted(gtcams):
            ...
            if k in tscams:
                ...
                start = tsd['FrameId'].min()
                end = tsd['FrameId'].max()
                tsd = tsd.set_index(['FrameId', 'Id'])
                tsds.append(tsd)
                frame_dict[f'c{str(k).zfill(3)}'] = [start, end]
        ...
        multiCamAcc = mm.utils.compare_to_groundtruth(gtds, tsds, 'iou')
        if mct:
            summary = mh.compute(multiCamAcc, metrics=metrics, name="MultiCam")
            
        else:
            acc_list = []
            cameras = frame_dict.keys()
            for frame in frame_dict.values():
                acc = multiCamAcc.events.loc[frame[0]:frame[1]]
                acc_list.append(acc)
            summary = mh.compute_many(acc_list, metrics=metrics, names=cameras, generate_overall=True)
...
if __name__ == '__main__':
    ...
    try:
        summary = eval(test, pred, mread=args.mread, dstype=args.dstype, roidir=args.roidir, mct=args.mct)
